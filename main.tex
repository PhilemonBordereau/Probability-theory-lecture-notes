...
\title{Basic probability theory 2025}
...
\noindent In this chapter we discuss some foundational notions of probability theory:
\begin{itemize}
...
Our first aim is to motivate the modern notion of a probability space or a probabilistic model. To do this let us consider two examples:
...
~\\
\paragraph{(1) Random number.} In the modern framework, to describe a random number mathematically, we use three inputs: 
\begin{itemize}
...
\begin{defn}[Finite probability space, Kolmogorov 1933 ]
A finite probability space is a triple $(\Omega, \F, \P)$, where
\begin{itemize}
...
Some natural examples where infinite sample spaces come in: 
\begin{itemize}
\item an uniform point on a line segment e.g. stemming from breaking a stick into several pieces; 
\item the position on the street where the first raindrop of the day falls; 
\item or the space of all infinite sequences of coin tosses. 
\end{itemize}
In all these cases the mathematically natural state space is even uncountable. Countably infinite state spaces can also come up: for example if we want to model the first moment that a repeated coin toss comes up heads, the value might be $1, 2, 3$ or with very very small probability also $10^{10}$, and thus a natural state space would contain all natural numbers.
...
There are of also other notions of equality for random variables:
\begin{itemize}
\item We say that two random variables $X, Y$ defined on the same probability space $(\Omega, \F, \P)$ are everywhere equal if for all $\omega \in \Omega$ we have that $X(\omega) = Y(\omega)$
\item  We say that two random variables $X, Y$ defined on the same probability space $(\Omega, \F, \P)$ are almost surely equal if $\P(\{\omega: X(\omega) = Y(\omega)\}) = 1$. Here, it of course first needs to be showed that $\{\omega: X(\omega) = Y(\omega)\}$ is an event, i.e. belongs to $\F$ - this is on the example sheet.
\end{itemize}
Clearly everywhere equality implies almost everywhere equality and the converse is false - e.g. consider the random variables $X(\omega) := \omega1_{\omega \neq 1/2}$ and $Y(\omega) := \omega$ on the probability space $([0,1], \F_B, \P_U)$. It is also clear that equality in law cannot imply almost sure equality, even if the random variables would be defined on the same probability space. Finally,

\begin{exo}
Let $X, Y$ be two random variables defined on the same probability space and almost surely equal. Then they are also equal in law.
\end{exo}
%Let us here see how they help us describe a slightly more complicated probabilistic model than coin tosses.
...
\begin{itemize}
\item Each $n$-tuple can be seen as an element of the product space $\{H, T\} \times \cdots \{H,T\}$, so we can use as $\Omega$ the product space. Let's denote also by $\Omega_0 = \{H, T\}$ the state spaces for the coordinates.
\item Observe that the power-set of $\Omega$ is also at the same time the smallest $\sigma-$algebra containing all sets of the form $E_1 \times \cdots \times E_n$ with each $E_i$ in the power-set of a single coordinate $\{H,T\}$
...
\begin{enumerate}
    \item The product $\sigma-$algebra $\F_\Pi$ is simple and natural: it is the smallest $\sigma-$algebra containing all $E_{i_1} \times \cdots \times E_{i_n}$ with $E_{i_j} \in \F_{i_j}$ for all $j = 1 \dots n$ and $\{i_j\}_{j = 1 \dots n}$ a finite subset of $\N$. We stress that it is not equal to the set of all sets of the form $E_{i_1} \times \cdots \times E_{i_n}$ even on a finite product space. \footnote{For example, even in the example above for $2$ coin tosses you can convince yourself that the set $\{(H, H),(T,T)\}$ is not of the product form. A similar thing happens in the product topology. Also notice that whereas the collection of all sets of the form $E_1 \times ... \times E_n$ with $E_i \in \F_i$ can be put canonically in a correspondence with $\F_1 \times \dots \F_n$ (one has to be careful with empty sets), they are formally not the same thing - first gives products of $n$ sets, the other $n$-tuples of sets.}
...
\begin{rem}
We will in fact mainly use it in the case where $(\Omega_i, \F_i)$ are the real numbers with its Borel $\sigma$-algebra. For such finite products the proof will be done in Analysis IV and in fact can be deduced also from the existence of Lebesgue measure on just $([0,1], \F_B)$! This will be on the non-examinable part of the example sheet.
\end{rem}

All these difficulties listed, it is actually rather easy to see the existence and uniqueness in the case of a finite number of discrete probability spaces, so let us do that. Below, we state it in the case where the $\sigma-$algebras are equal to the power set, but as discussed before (see Proposition \ref{prop:disatom}, this essentially encompasses the case of general $\sigma-$algebras on discrete spaces.
...
We admit this theorem in the general case, but let us sketch how the existence part of theorem follows from the existence of the uniform measure on $([0,1], \F_E)$. This method is also used in modelling random variables on computers:

\begin{proof}[Sketch of how to construct random variables from the uniform random variable:]
Suppose we are given a cumulative distribution function $F$. The idea is to construct the random variables using the probability space $P_U$ on $((0,1], \F_{E}, \P_U)$, i.e. the unit interval with the uniform measure. We aim to find a suitable map from the interval to $\R$ that maps the uniform random variable $X_U$ to the one we aim to construct.

To do this define $X_F: (0,1] \to \R$ by $$X_F(x) := \inf_{y \in \R}\{F(y) \geq x\}.$$ Then clearly $X_F$ is non-decreasing and in fact this suffices (see the exercise sheet) for it to be measurable from $((0,1], \F_{E})$ to $(\R, \F_E)$. Hence $X_F$ is a random variable \footnote{It is sometimes called the generalized quantile function and can be seen as defining the left-continuous inverse of the function $F$}.

But now 
$$\P_U(X_F \in (-\infty,x]) = \P_U((0,\sup_{z \in (0,1]}\{z < F(x)\})) = \P_U((0,F(x)]) = F(x)$$
and hence indeed $F$ is the cumulative distribution function of the random variable $X_F$. 
...
In the other extreme $F_X$ could also be everywhere continuous. These observations are behind singling out discrete and continuous random variables:
...
These definitions look a bit abstract / non-telling from the probabilistic perspective. The following exercise redefines them in a different way:
...
%The proof, which is really not difficult, but just not central to the story, is left as a non-examinable exercise.
...
As mentioned already, a random variable that takes only values $\{0,1\}$, taking value $1$ with probability $p$ is called a Bernoulli random variable of parameter $p$. It is named after the Swiss mathematician Jacob Bernoulli, who also thought that all sciences need mathematics, but mathematics doesn't need any. Leaving you to judge, let us see that these examples come up very often.
...
The part that geometric r.v. is memoryless is on the example sheet.
...
For example:
\begin{itemize}
    \item Consider on the one hand $(X_1, X_2)$, where both $X_1$ and $X_2$ encode independent fair coin tosses. 
    \item On the other hand, consider $(X_1, \widetilde X_2)$, where $X_1$ is a fair coin toss, but $\widetilde X_2$ is heads when $X_1$ is tails and $\widetilde X_2$ is tails if $X_1$ is heads. 
\end{itemize}
    Then the marginal laws of the vector $(X_1, X_2)$ and $(X_1, \widetilde X_2)$ are the same (why?), yet they clearly describe very different situations! 
...
Proving by hand that the sum of two random variables $X_1$ and $X_2$ is a random variable requires quite a bit of patience - e.g. think of the case of Bernoulli random variables. But given the Lemma before, it becomes quite easy.

\begin{proof}
By above $\overline X$ is a measurable function from $(\Omega, \F)$ to $(\R^n, \F_B)$. But now $\Phi: \R^n \to \R$ given by $\Phi(\overline x) = \sum_{i = 1}^n a_i x_i$ is continuous from $(\R^n, \tau_B)$ to $(\R, \tau_B)$ and in particular it is measurable because by the definition of continuity each preimage of an open set is open!

Further, it is a direct check that a concatenation $f_2 \circ f_1$ of measurable maps $f_1: (\Omega, \F) \to (\Omega_1, \F_1)$, $f_2: (\Omega_1, \F_1) \to (\Omega_2, \F_2)$ is $(\Omega, \F) \to (\Omega_2, \F_2)$-measurable. Thus $\sum_{i=1}^n a_i X_i = \Phi(\overline X)$ is measurable from $(\Omega, \F)$ to $(\R,\tau_E)$ and hence a random variable.
\end{proof}


\subsubsection{Joint cumulative distribution function}
Similarly to the case of a single random variable, it now natural to ask for ways to characterise the laws of random vectors. In analogy with the oned-dimensional case, given a random vector $\bar X$ we can consider the functions $F: \R^n \to [0,1]$ given by $
F_{\bar X}(t_1, \dots, t_n) = \P_{\bar X}((-\infty, t_1] \times \dots \times (-\infty, t_n])$. These functions are called joint cumulative distribution functions and are defined as follows.
...
Any function $F: \R^n \to [0,1]$ is called a joint cumulative distribution function (c.d.f.), if it satisfies the following conditions:
	\begin{enumerate}
		...
		\item $F$ is right-continuous, meaning that for any sequence $(x_1^m, \dots, x_n^m)_{m \geq 1}$ such that for all $m\geq 1$ we have that $x_i^m \geq x_i$ and $x_i^m \to x_i$ as $m \to \infty$, it holds that $F(x_1^m, \dots, x_n^m) \to F(x_1, \dots, x_n)$.
        \item For each $\bar y^1 := (y_1^1, \dots, y_n^1)$ and $\bar y^0 := (y_1^0, \dots, y_n^0)$ with $y_i^1 \leq y_i^0$ for all $i = 1 \dots n$, we have the following Inclusion-Exclusion inequality: 
        $$\sum_{S \subseteq \{1, \dots, n\}}(-1)^{|S|}F(y_1^{1_{1\in S}},\dots, y_n^{1_{1\in S}}) \geq 0.
        $$
	\end{enumerate}
\end{defn}
...
The condition $(5)$ seems new and maybe scary to begin with but it just guarantees that for every box $B :) [y_1^1, y_2^0] \times \dots \times [y_n^1, y_n^0]$ we have that $\P_{\bar X}(B) \geq 0$.
We will see on the exercise sheet that this condition is necessary to have the desired one-to-one correspondence between the joint c.d.f.-s and probability measures. In the case $n = 1$ it is automatically satisfied because of monotonicity. 

This connection comes in two results lie before. One direction is again easy:
...
When $\overline \mu = 0$ and $C$ is the $n \times n$ identity matrix $I_n$, we call the law $\Nn(0,I_n)$ the standard Gaussian in $\R^n$. 

Gaussian vectors marry well with linear algebra: if we apply a linear transformation to a Gaussian vector, then it remains Gaussian. In fact all Gaussian vectors in $\R^n$ are given by just linear transformations of the standard Gaussian.\\
...
For a concrete lively example, let's go back to the Erdos-Renyi random graph on $n$ vertices, where each edge is independently included with probability $p$. We can then fix some vertex $v$ and consider the random variable 